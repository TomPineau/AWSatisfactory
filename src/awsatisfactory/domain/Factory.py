...
class Factory:
    ...
    def __init__(self, name: str, inventory: dict = {}, lines: list[Line] = []) -> None:
        self.name: str = name
        ...
        for line in self.get_lines():
            for machine in line.get_machines():
                ...
                for machine_type, power in result["power_consumption"].items():
                    self.power_consumption[machine_type] = (
                        self.power_consumption.get(machine_type, 0) + power
                    )

                print(
                    f"Machine: {machine.get_name()}, elapsed_craft_time = {machine.get_elapsed_craft_time()}, craft_time = {machine.get_recipe().get_craft_time() / machine.get_overclock()}, craft_nb = {result['craft_nb']}, power_consumption = {result['power_consumption']}"
                )
                self.show_inventory()
