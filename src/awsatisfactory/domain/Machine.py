...
class Machine:
    ...
    def __init__(self,
        ...
        if recipe.get_machine_type() != machine_type:
            raise ValueError(f"Machine {name} of type {machine_type} cannot be assigned recipe {recipe.get_name()} of type {recipe.get_machine_type()}.")
    ...
    def print(self) -> None :
        print(f"Machine : name = {self.name}, machine_type = {self.machine_type}, line = {self.get_line().get_name()}, base_power = {self.base_power}, overclock = {self.overclock}, elapsed_craft_time = {self.elapsed_craft_time}")
    ...
    def simulate(self, inventory : dict, minutes : int) -> None :
        ...
        inputs : dict = {item: quantity * craft_nb for item, quantity in recipe_inputs.items()}
        outputs : dict = {item: quantity * craft_nb for item, quantity in recipe_outputs.items()}
